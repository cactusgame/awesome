...
k_metric = 'hma20'
df.loc[:, ['close', k_metric]].plot(secondary_y=True, grid=True, figsize=(10, 6))
plt.savefig('books_read.png')
plt.show()

df['k'] = df[k_metric] - df[k_metric].shift(1)
df['buy'] = np.where(df['k'] > 0, 1, 0)
df['sell'] = np.where(df['k'] < 0, 1, 0)
df['close'] = df['close'] / 1000
df = df.loc[:, ['trade_date', 'close', 'hma5', 'k', 'buy', 'sell']]
print df
