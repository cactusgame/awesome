...
class Trainer:
    ...
    def train(self):
        ...
        # eval twice. Because I want to see the difference between 2 differ eval set.
        eval_train_input_fn = self.model.make_training_input_fn(
            tf_transform_output,
            train_tfrecord_fname_out + '*',
            EVAL_BATCH_SIZE)
        eval_eval_input_fn = self.model.make_training_input_fn(
            tf_transform_output,
        ...
        # eval_trainset_result = estimator.evaluate(eval_train_input_fn,
        #                                           steps=20,
        #                                           name='eval_train')
        eval_evalset_result = estimator.evaluate(eval_eval_input_fn, steps=100,
                                                 name='eval_eval')
        print eval_evalset_result

        estimator.export_savedmodel(TARGET_DIR, make_serving_input_fn, strip_default_attrs=True)
